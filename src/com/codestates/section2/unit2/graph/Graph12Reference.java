...
// 2022.9.26(월) 15h50
public class Graph12Reference {
    public int connectedVertices(int[][] edges) {

        final int[] bigger = {0}; // 나의 질문 = 왜 final로 선언해둘까? -> 주소값만 고정
        ...
        boolean[] visited = new boolean[bigger[0] + 1]; // boolean 자료형의 기본 값 = false, 방문 여부 초기화된 상태
        // 컴포넌트가 몇 개인지 카운트할 변수를 선언합니다.
        int count = 0;

        // 방문 여부를 체크한 배열을 모두 순회합니다. 모든 요소 방문해야 하는 문제로써 for문 0부터 시작
        for (int vertex = 0; vertex < visited.length; vertex++) {
            //만약 방문하지 않았다면
            if (!visited[vertex]) {
                //컴포넌트를 늘려주고 <- 시작할 때 1개는 늘여줘야 함
                count++;
